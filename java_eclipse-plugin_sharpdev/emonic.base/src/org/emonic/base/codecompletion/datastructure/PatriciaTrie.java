 ...
 * are performed at worst in O(K) time, where String is the number of bits in the
 ...
 * (at most String of them, described above) will perform a single bit comparison
 ...
 * <code>ClassCastException<code> if the method is expecting an instance of String 
 * (and it isn't K).
 * 
 * <pre>
 ...
 */
public class PatriciaTrie extends AbstractMap implements Trie, Serializable, ITrie {
    ...
    protected final String asKey(Object key) {
        try {
            return (String)key;
     ...
     */
    private TrieEntry getNearestEntryForKey(String key, int keyLength) {
        TrieEntry current = root.left;
        TrieEntry path = root;
        while(true) {
            if(current.bitIndex <= path.bitIndex)
    ...
    private boolean selectR(TrieEntry h, int bitIndex, 
            final String key, final int keyLength, final TrieEntry[] result) {
    ...
    //@SuppressWarnings("unchecked")
    public Map.Entry  select(String key, Cursor cursor) {
        int keyLength = length(key);
        ...
        if (h.bitIndex <= bitIndex) {
            if(!h.isEmpty()) {
                ...
                case Cursor.REMOVE_AND_EXIT:
                    TrieEntry entry = new TrieEntry((String)h.getKey(), (String)h.getValue(), -1);
                    result[0] = entry;
                    removeEntry(h);
                    return false;
     ...
     * @param key
     */
    public SortedMap getPrefixedBy(String key) {
     ...
     * @param length
     */
    public SortedMap getPrefixedBy(String key, int length) {
     ...
     * @param length
     */
    public SortedMap getPrefixedBy(String key, int offset, int length) {
     ...
     * @param bitLength
     */
    public SortedMap getPrefixedByBits(String key, int bitLength) {
     ...
     * @param length
     */
    private SortedMap getPrefixedByBits(String key, int offset, int length) {
     ...
     * to remove)
     */
    private String removeEntry(TrieEntry h) {
        if (h != root) {
            if (h.isInternalNode()) {
    ...
    public Map.Entry traverse(Cursor cursor) {
        ...
        while(entry != null) {
            ...
            case Cursor.REMOVE_AND_EXIT:
                Map.Entry value = new TrieEntry((String)current.getKey(), (String)current.getValue(), -1);
                removeEntry(current);
                return value;
     ...
     */
    private boolean isBitSet(String key, int keyLength, int bitIndex) {
     ...
     */
    private int bitIndex(String key, String foundKey) {
    ...
    private static class TrieEntry  implements Map.Entry, Serializable {
        ...
        private TrieEntry(String key, String value, int bitIndex) {
            this.key = key;
            this.value = value;
    ...
    public static interface KeyAnalyzer extends Comparator, Serializable {
         ...
         */
        public int bitIndex(CharSequence key, int keyStart, int keyLength,
        			CharSequence found, int foundStart, int foundLength);
    ...
    private class PrefixEntryIterator extends NodeIterator {
        ...
        PrefixEntryIterator(TrieEntry startScan, String prefix, int offset, int length) {
            subtree = startScan;
    ...
    private class SubMapEntryIterator extends NodeIterator {
        private final String firstExcludedKey;
     ...
     */
    protected TrieEntry  higherEntry(String key) {
     ...
     */
    protected TrieEntry  ceilingEntry(String key) {
        // Basically:
     ...
     */
    protected TrieEntry  lowerEntry(String key) {
        // Basically:
        // Follow the steps of adding an entry, but instead...
        //
        // - If we ever encounter a situation where we found an equal
     ...
     */
    protected TrieEntry  floorEntry(String key) {        
        // TODO: Cleanup so that we don't actually have to add/remove from the
        //       tree.  (We do it here because there are other well-defined 
        //       functions to perform the search.)
        int keyLength = length(key);
     ...
     */
    private TrieEntry subtree(String prefix, int offset, int length) {
        TrieEntry current = root.left;
        TrieEntry path = root;
        while(true) {
        ...
        if(entry == root && length((String)entry.getKey()) < offsetLength)
            return null;
        ...
        int bitIndex = keyAnalyzer.bitIndex(prefix, offset, length,
                                            entry.key, 0, length((String)entry.getKey()));
    ...
    private class PrefixSubMap extends SubMap {
        ...
        PrefixSubMap(String prefix, int offset, int length) {
            this.prefix = prefix;
            this.offset = offset;
            this.length = length;
            fromInclusive = false;
        ...
        public Object firstKey() {
            ...
            if (e == null || !keyAnalyzer.isPrefix(prefix, offset, length, (String)first))
                throw new NoSuchElementException();
            return first;
        }
        ...
        public Object lastKey() {
            ...
            if (e == null || !keyAnalyzer.isPrefix(prefix, offset, length, (String)last))
                throw new NoSuchElementException();
            return last;
        }
        ...
        protected boolean inToRange(String key, boolean forceInclusive) {
            return keyAnalyzer.isPrefix(prefix, offset, length, key);
        }
        
        protected boolean inFromRange(String key, boolean forceInclusive) {
            return keyAnalyzer.isPrefix(prefix, offset, length, key);
        }
        ...
        private void fixup() {
            ...
            if(modCount != keyModCount) {
                ...
                fromKey = entry == null ? null : (String)entry.getKey();
                if(fromKey != null) {
                    TrieEntry prior = previousEntry((TrieEntry)entry);
                    fromKey = prior == null ? null : (String)prior.getKey();
                }
                ...
                toKey = entry == null ? null : (String)entry.getKey();
    ...
    private class SubMap extends AbstractMap  implements SortedMap , java.io.Serializable {
        ...
        public boolean containsKey(Object key) {
            return inRange((String) key) && PatriciaTrie.this.containsKey(key);
        }
       
        //@SuppressWarnings("unchecked")
        public Object remove(Object key) {
            if(!inRange((String)key))
                return null;
        ...
        public Object firstKey() {
            ...
            Object first = e != null ? e.getKey() : null;
            if (e == null || toKey != null && !inToRange((String)first, false))
                throw new NoSuchElementException();
            return first;
        }
        ...
        public Object lastKey() {
            ...
            Object last = e != null ? e.getKey() : null;
            if (e == null || fromKey != null && !inFromRange((String)last, false))
                throw new NoSuchElementException();
            return last;
        }
    	...
    	public SortedMap headMap(Object toKey) {
    		// TODO Auto-generated method stub
    		System.out.println("PatriciaTrie:EntrySetView:headMap not implemented");
    		return null;
    	}

    	public SortedMap subMap(Object fromKey, Object toKey) {
    		// TODO Auto-generated method stub
    		System.out.println("PatriciaTrie:EntrySetView:subMap not implemented");
    		return null;
    	}

    	public SortedMap tailMap(Object fromKey) {
    		// TODO Auto-generated method stub
    		System.out.println("PatriciaTrie:EntrySetView:tailMap not implemented");
    		return null;
    	}
    }
	...
	public SortedMap headMap(Object toKey) {
		// TODO Auto-generated method stub
		System.out.println("PatriciaTrie:headMap not implemented");
		return null;
	}

	public SortedMap subMap(Object fromKey, Object toKey) {
		// TODO Auto-generated method stub
		System.out.println("PatriciaTrie:subMap not implemented");
		return null;
	}

	public SortedMap tailMap(Object fromKey) {
		// TODO Auto-generated method stub
		System.out.println("PatriciaTrie:tailMap not implemented");
		return null;
	}
	...
	// NEEDED FOR ITRIE
	public Map getSortedMap(String searchstring) {
		return getPrefixedBy(searchstring);
	}
