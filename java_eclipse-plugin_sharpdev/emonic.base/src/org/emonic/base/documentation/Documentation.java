	...
	public Documentation(String name, Node element) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (nodeName.equals(XML_SUMMARY_TAG)) {
				...
				exceptionDeclarations.add(new ExceptionDocumentation(
						(Element) node));
		...
		params = (ParameterDocumentation[]) parameters
				.toArray(new ParameterDocumentation[parameters.size()]);
		exceptions = (ExceptionDocumentation[]) exceptionDeclarations
				.toArray(new ExceptionDocumentation[exceptionDeclarations
						.size()]);
	}
